...
export function GameComponent() {
  ...
  const shareToSocialMedia = React.useCallback((platform: string) => {
    const message = "🏆 Just claimed some coins in Baby Cheetah! 🚀 Join me this exciting crypto game and start earning too! 🤑 Complete tasks, invite friends, rise the ranks. Let's get those together! 💰🐾"
          ...
          >
            <motion.div
              ...
              style={{
                ...
                boxShadow: "0 10px rgba(102, 126, 234, 0.5)"
              }}
            />
          </motion.div>
        </div>
        ...
        {premiumShopItems.map((item) => (
          ...
          >
            <NeonGradientCard className="bg-gradient-to-br from-gray-900 to-black text-white overflow-hidden transform transition-all duration-300 hover:shadow-2xl">
              ...
              <CardContent className="p-4">
                <motion.div 
                  className="w-full h-40 mb-4 relative overflow-hidden rounde d-lg"
                  whileHover={{ scale: 1.1 }}
                  transition={{ type: "spring", stiffness: 300, damping: 10 }}
                >
                  <Image
